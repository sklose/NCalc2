...
namespace NCalc.Tests
{
    public class PerformanceAsync
    {
        ...
        public async void Arithmetics(string formula)
        {
            var expression = new Expression(formula);
            var lambda = expression.ToLambda<object>();

            var m1 = Measure(async() => await expression.EvaluateAsync());
        ...
        public async void ParameterAccess(string formula)
        {
            var expression = new Expression(formula);
        ...
        [Theory]
        [InlineData("[Param1] * 7 + [Param2]")]
        public async void DynamicParameterAccess(string formula)
        {
            var expression = new Expression(formula);
            var lambda = expression.ToLambda<Context, int>();
        ...
        public void FunctionWithDynamicParameterAccess(string formula)
        {
            ...
            expression.EvaluateFunctionAsync += async(name, args) =>
            {
                if (name == "Foo")
                {
                    var param =  await args.EvaluateParametersAsync();
